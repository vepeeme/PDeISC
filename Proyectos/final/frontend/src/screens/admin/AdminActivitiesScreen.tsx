// src/screens/admin/AdminActivitiesScreen.tsx
...
const AdminActivitiesScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  const [actividades, setActividades] = useState<Actividad[]>([]);
  const [actividadesFiltered, setActividadesFiltered] = useState<Actividad[]>([]);
  ...
  //  Filtrar cuando cambie el filtro o la búsqueda
  useEffect(() => {
    applyFilters();
  }, [filter, searchQuery, actividades]);
  ...
  const applyFilters = () => {
    ...
    // Filtro por estado
    if (filter !== 'all') {
      filtered = filtered.filter(act => act.estado === filter);
    }
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(act =>
        ...
        (act.descripcion && act.descripcion.toLowerCase().includes(query)) ||
        (act.area_nombre && act.area_nombre.toLowerCase().includes(query))
      );
    }

    setActividadesFiltered(filtered);
  };
  ...
  const handleDelete = (actividad: Actividad) => {
    Alert.alert(
      'Eliminar Actividad',
      `¿Estás seguro de eliminar "${actividad.titulo}"?`,
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await actividadesService.delete(actividad.id);
    ...
    >
      ...
      <View style={styles.activityMeta}>
        ...
        <View style={styles.metaRow}>
          ...
          <Text style={styles.metaText}>{item.encargado_nombre || 'Sin encargado'}</Text>
        </View>
      </View>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.searchContainer}>
        ...
        <TextInput
          ...
          placeholder="Buscar actividades por título, área..."
          value={searchQuery}
      ...
      {/*  FILTROS CON SCROLL HORIZONTAL Y TODOS LOS ESTADOS */}
      <ScrollView
        horizontal
